...
void x4driver_data_ready(void)
{
    ...
    if (XEP_ERROR_X4DRIVER_OK == status)
    {
        ...
        // for (int i = 0; i<(sizeof(data_frame_normalized) / sizeof(data_frame_normalized[0])); i++){
        //     for (int j = 0; j< fdata_count; j++){
        //         fprintf(f, "%lf%s",data_frame_normalized[i,j],(j<fdata_count-1?",":""));
        //     }
        //     fprintf(f,"\n");
        // }
...
int taskRadar(void)
{
    ...
    status = x4driver_set_tx_center_frequency(x4driver, (xtx4_tx_center_frequency_t)3);
    ...
    status = x4driver_set_frame_area(x4driver, 0.3, 8.0); // Observe from 0.5m to 4.0m.
