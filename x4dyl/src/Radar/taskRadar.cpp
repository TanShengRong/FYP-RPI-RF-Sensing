...
int taskRadar(void)
{
    ...
    status = x4driver_set_sweep_trigger_control(x4driver, SWEEP_TRIGGER_MANUAL); // MANUAL By default let sweep trigger control done by X4
#ifdef DEBUG
    if(status == XEP_ERROR_X4DRIVER_OK)
    {
...
/*
 ...
 /*   digitalWrite(IOctl, LOW);
    ...
    for (;;)
    {
        ...
        if((end.tv_sec - start.tv_sec )> 60)  //十秒
        {
            gettimeofday(&start,NULL); //gettimeofday(&start,&tz);结果一样
            system("date \"+%Y-%m-%d %H:%M:%S\"");
            printf("fps:%d\n",(frameend -  framestart )/60);
